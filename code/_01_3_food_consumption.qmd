...
add_area_department <- function(df){
  ...
  mutate(
    treatment_group = case_when(
      ...
      treated_status == "never_treated" ~ "control_grp",
      TRUE ~ NA_character_
    ) %>%
    labelled(
      labels = c(
...
svy_descriptive <- function(design, myVar, by_vars) {
  ...
  area_inc_counts_df <- area_inc_counts_df %>%
    select(by_vars, variable, total_count, na_count) %>%
    pivot_wider(
      names_from = variable,
  ...
  summary_stats <- area_inc_means %>%
    left_join(area_inc_counts_df, by = by_vars)
...
summary_stats_gt <- svy_descriptive(
      enaho_design, 
      myVar=myVar_analysis, 
      by_vars=c("region", "rural_urban")
)
