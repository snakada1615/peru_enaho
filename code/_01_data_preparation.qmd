...
for(year_current in yearlist){
  ...
  df_food_clean <- df_food_clean %>%
    bind_rows(
      enaho_design_clean$variables
    )
  df_food_org <- df_food_org %>%
    bind_rows(
      enaho_design$variables
    )
...
# 全年分結合データ保存
saveRDS(df_food_clean, file = file.path(gdrive_dir, "output",
    "df_foodgrp_consumption_all_clean.rds"))
