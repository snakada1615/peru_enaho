...
area_inc_counts_df <- area_inc_counts_df %>%
  select(department, inc_quintile, variable, total_weighted, na_weighted) %>%
  pivot_wider(
    names_from = variable,
...
svy_descriptive <- function(design, myVar, by_vars) {
  ...
  rm(area_inc_means, area_inc_vars, area_inc_counts_df, area_inc_counts_list,
      diagonal_indices, variance_cols, df_data, weights_vec, by_formula, 
      main_formula, n_vars)

  return(summary_stats)
}

summary_stats_clean <- svy_descriptive(enaho_design_clean, myVar, 
                                            c("department", "inc_quintile"))
