...
for(var in myVar) {
  cat("外れ値検出中:", var, "\n")
  outlier_results[[var]] <- detect_outliers_simple(enaho_design, var)
}
...
for(var in myVar) {
  outlier_flag <- outlier_results[[var]]$is_outlier
  clean_data[outlier_flag & !is.na(outlier_flag), var] <- NA
}
