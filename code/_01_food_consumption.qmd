    ...
    mutate(
      ...
      labelled(
        labels = c(
          ...
          "Monthly" = 0.5,
          "Twice a week" = 4.29,
          "Three times a week" = 6.43,
          "Default (NA or others)" = 15
        ),
      ...
      labelled(
        labels = c(
          "bread (grams)" = 300,
          "gram" = 1,
          "large sliced bread loaf" = 800,
          "small sliced bread loaf" = 400,
          "small sponge cake" = 200,
          "kilogram" = 1000,
          "liter" = 1030,
          "large_can(milk)" = 500,
          "small_can(milk)" = 250,
...
mygroup <- c("department","rural_urban", "inc_quintile")
...
# (3) 新しい構造から値を正しく抽出
# # 方法1: 変数名でアクセス
# quantile_values <- as.numeric(income_q$income_net)
# print("Extracted quantile values (method 1):")
# print(quantile_values)

# 方法2: coef()関数使用（推奨）
quantile_values <- coef(income_q)
print("Extracted quantile values (method 2 - coef):")
print(quantile_values)

# (4) 境界ベクトル作成
breaks <- c(-Inf, quantile_values, Inf)
print("Breaks for cut():")
print(breaks)

# (5) 所得5分位の作成
df_foodgrp_consumption_capita$inc_quintile <- cut(
  df_foodgrp_consumption_capita$income_net,
  breaks = breaks,
  labels = paste0("Q", 1:5),  # 5個のラベルに修正
  include.lowest = TRUE
)

# 結果確認
table(df_foodgrp_consumption_capita$inc_quintile, useNA = "ifany")
