...
myfactor <- "FACTOR07"  # 年度に応じたウェイト変数名へ置換
...
enaho_design <- svydesign(
  id = ~ CONGLOME,
  strata = ~ ESTRATO,
  weights = ~ myfactor,   # 年度に応じたウェイト変数名へ置換
  data = df_foodgrp_consumption_capita,
  nest = TRUE
)

# (2) 基本統計量の確認（survey package使用）
# 平均とSE
m <- svymean(as.formula(paste("~", paste(myVar, collapse = "+"))), design = enaho_design, na.rm = TRUE)
print(m)
...
# 中央値や分位点（ロバストな代表値）
# describeの中央値・四分位数は svyquantile で取得
q <- svyquantile(
  as.formula(paste("~", paste(myVar, collapse = "+"))),
  design = enaho_design,
  quantiles = c(0.25, 0.5, 0.75),
  ci = TRUE, na.rm = TRUE, interval.type = "Wald"
)
print(q)
